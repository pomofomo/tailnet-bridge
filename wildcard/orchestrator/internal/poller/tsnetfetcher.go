// Package poller — tsnet-backed Fetcher (SPEC §10.3 step 4).
//
...
// Fetch calls return that error until the bridge restarts.
...
func (f *TsnetFetcher) Fetch(ctx context.Context, communityID, url, prevETag, expectedDomain string) (*directory.FetchResult, error) {
	f.mu.RLock()
	...
	return directory.Fetch(ctx, hc, url, prevETag, expectedDomain)
}
